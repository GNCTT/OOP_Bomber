...
//            explodes.add(new Explode(dx + 1, dy, Sprite.movingSprite(Sprite.explosion_horizontal_right_last, Sprite.explosion_horizontal_right_last1,Sprite.explosion_horizontal_right_last2, animate, 20).getFxImage()));
            ...
            if (!(top instanceof Brick) && BombermanGame.map.getObject(dx, dy - 1) instanceof Grass) {
                explodes.add(new Explode(dx, dy - 1, 5));
                Entity topp = BombermanGame.map.getEntity(dx, dy - 2);
                System.out.println(topp);
                if (topp instanceof Brick) {
                    topp.setBeDestroy(true);
                }
                if (!(topp instanceof Brick) && BombermanGame.map.getObject(dx, dy - 2) instanceof Grass) {
                    explodes.add(new Explode(dx, dy - 2, 0));
                }
            }
            if (left instanceof Brick) {
                left.setBeDestroy(true);
            ...
            if (!(left instanceof Brick) && BombermanGame.map.getObject(dx - 1, dy) instanceof Grass) {
                explodes.add(new Explode(dx - 1, dy, 6));
                Entity leftt = BombermanGame.map.getEntity(dx - 2, dy);
                if (leftt instanceof Brick) {
                    leftt.setBeDestroy(true);
                }
                if (!(leftt instanceof Brick) && BombermanGame.map.getObject(dx - 2, dy) instanceof Grass) {
                    explodes.add(new Explode(dx - 2, dy, 3));
                }
            }
            if (right instanceof Brick) {
                right.setBeDestroy(true);
            ...
            if (!(right instanceof Brick) && BombermanGame.map.getObject(dx + 1, dy) instanceof Grass) {
                explodes.add(new Explode(dx + 1, dy, 6));
                Entity rightt = BombermanGame.map.getEntity(dx + 2, dy);
                if (rightt instanceof Brick) {
                    rightt.setBeDestroy(true);
                }
                if (!(rightt instanceof Brick) && BombermanGame.map.getObject(dx + 2, dy) instanceof Grass) {
                    explodes.add(new Explode(dx + 2, dy, 1));
                }
            }
            if (bot instanceof Brick) {
                bot.setBeDestroy(true);
            ...
            if (!(bot instanceof Brick) && BombermanGame.map.getObject(dx, dy + 1) instanceof Grass) {
                explodes.add(new Explode(dx, dy + 1, 5));
                Entity bott = BombermanGame.map.getEntity(dx, dy + 2);
                if (bott instanceof Brick) {
                    bott.setBeDestroy(true);
                }
                if (!(bott instanceof Brick) && BombermanGame.map.getObject(dx, dy + 2) instanceof Grass) {
                    explodes.add(new Explode(dx, dy + 2, 2));
                }
            }
